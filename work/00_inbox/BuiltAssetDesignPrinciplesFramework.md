...
5. Pattern Index
6. Pattern bodies `BDPF.1..BDPF.11` with lettered extensions
...
| Услугу сопровождения нужно начать с анализа ТЗ, участка, ТУ, изысканий и иных исходных carriers | `BDPF.3B` | InitialDataAnalysisPackage@Project с carrier inventory, source signals, gaps, P2S cards и readiness gate. |
...
FrameworkCarrierStructureCapture@BuiltAssetDesignPrinciplesFramework:
  evaluatedCarrierRef: BuiltAssetDesignPrinciplesFramework.md@draft-v0.4
...
`BuiltAssetDesignPrinciplesFramework.md` является publication carrier-ом текущей DPF-редакции, а не всей архитектурой DPF-пакета, не source pack, не quality result и не work authority.
...
DPFPackageStewardshipRecord@BuiltAssetDesignPrinciplesFramework:
  ...
  patternSetRefs: BDPF.1..BDPF.11 with BDPF.3A, BDPF.3B, BDPF.6A, BDPF.6B, BDPF.6C, BDPF.7A, BDPF.7B
  relationRecordRefs: PFR-BDPF-*
  publicationCarrierRefs: BuiltAssetDesignPrinciplesFramework.md@draft-v0.4
...
PrincipleFrameworkArchitectureDecision@BuiltAssetDesignDPF:
  ...
  selectedPatternSetRefs: BDPF.1..BDPF.11 with BDPF.3A, BDPF.3B, BDPF.6A, BDPF.6B, BDPF.6C, BDPF.7A, BDPF.7B
...
| Услугу сопровождения нужно начать с анализа ТЗ, участка, ТУ, изысканий и иных исходных carriers | BDPF.3B InitialDataAnalysisPackage@Project | Пакет анализа исходных данных с carrier inventory, source signals, gaps, initial use/system extraction, P2S cards и readiness gate. |
...
| BDPF.3B | Анализ исходных данных как стартовая услуга сопровождения | Когда сопровождение начинается с анализа ТЗ, участка, ТУ, изысканий, обследований и других исходных carriers | `InitialDataAnalysisPackage@Project` | C.32.P2S, C.30, E.4.DPF, E.4.PFR, A.10, B.3, A.21 |
...
Если сопровождение начинается через `BDPF.3B`, `BDPF.3` остаётся паттерном одной записи source-signal use. Он включается в `InitialDataAnalysisPackage@Project`, но не заменяет весь стартовый пакет услуги.
...
`BDPF.3A` остаётся паттерном одной P2S-карты от problem-side материала к структурам ОКС. Если он используется в `BDPF.3B`, то пакет только собирает несколько таких карт вместе с carriers, gaps и readiness; он не превращает P2S-карту в PAD или candidate configuration.
...
Проектировщик проводит анализ. Экспертиза отслеживает, что анализ предъявлен в форме, пригодной для дальнейшей P2S-цепочки.
...
5. какая `architectureStructuringNeed` возникла как мост от problem-side материала к architecture work;
6. какие пробелы, противоречия и source-return conditions остаются;
7. какой следующий BDPF/FPF-owner должен принять claim;
8. какой readiness status допускает или блокирует следующий шаг.

Рабочая трасса пакета:
...
`BDPF.3B` не заменяет `BDPF.3` и `BDPF.3A`. Он композиционно использует записи source-signal use и P2S-карты как стартовый сервисный пакет.

`InitialDataAnalysisPackage@Project` не является PAD, proof/evidence sufficiency, work authorization или экспертной заменой анализа проектировщика. `SourceAnalysisReadinessGate@Project` только фиксирует готовность следа анализа к следующему проектному шагу.
...
DesignReadinessGate:
  gateId:
  gateProfile:
    domainDecisionReadiness | sourceAnalysisReadiness | publicationReadiness | workEntryReadiness | reviewReadiness
  designStep:
  objectScope:
  requiredRecords:
  requiredEvidence:
...
`sourceAnalysisReadiness` проходит, если `SourceCarrierInventory@Project`, `DesignSourceSignalUseRecord@Project[*]`, `SourceGapAndAmbiguityRegister@Project`, initial use/system extraction и `BuiltAssetProblemToStructureFlowCard@InitialDataAnalysis[*]` достаточно предъявлены для перехода к BDPF.4/5/6 или явно остановлены. Gate decision: `pass | passWithReservations | hold | reject | abstain`.

`SourceAnalysisReadinessGate@Project` не выбирает проектное решение и не доказывает его допустимость. Он проверяет, собран ли достаточный след анализа исходных данных для следующего шага услуги.
...
| `BDPF-P2S-3A` | Если сопровождение стартует с анализа исходных данных, предъявлен `InitialDataAnalysisPackage@Project`, а не свободный отчёт: carriers, source signals, gaps, initial use/system extraction, P2S cards и readiness gate связаны между собой. |
| `BDPF-P2S-3B` | `SourceAnalysisReadinessGate@Project` имеет status `pass/passWithReservations/hold/reject/abstain` и не используется как proof, PAD или разрешение на проектное решение. |
...
SelectedBDPFApplication:
  ...
  BDPF.3B InitialDataAnalysisPackage@Project
...
PatternFrameworkRelationRecord@BuiltAssetDesignDPF:
  relationId: PFR-BDPF-INITIAL-DATA-ANALYSIS-001
  sourceRef: BDPF.3B
  targetRef: BDPF.3, BDPF.3A, BDPF.4, BDPF.5, BDPF.6, BDPF.9, A.10, B.3, A.21, C.32.P2S
  relationFunction: Service-start composition and source-analysis readiness routing
  governedUse: стартовый пакет анализа исходных данных для перехода к use/system/architecture work
  directGoverningPatternRef: E.4.PFR; C.32.P2S for route; A.21 for readiness boundary
  blockedStrongerReading: InitialDataAnalysisPackage is not PAD, not evidence sufficiency, not work authorization, not expert replacement of designer analysis
  sourceReturnCondition: source carrier/version/owner/currentness/gap change
  refreshOrSupersessionCondition: refresh when initial-data analysis slots, readiness gate profile, or source-currentness assumptions change
...
SourceUseAndRefreshMap@BuiltAssetDesignDPF:
  ...
  sourceRows:
    - sourceFamily: FPF Core
      ...
      affectsPatterns: BDPF.1..BDPF.11 with lettered extensions
    ...
    - sourceFamily: project source documents and requirements
      ...
      affectsPatterns: BDPF.3, BDPF.3A, BDPF.3B, BDPF.4, BDPF.5, BDPF.6, BDPF.9, BDPF.11
...
```text
AcceptanceCase-BDPF-00-InitialDataAnalysis:
  sourcePressure: ТЗ / участок / ТУ / изыскания / обследования -> старт услуги сопровождения
  forcedDistinction: initial-data analysis package is a traceable service-start package, not a free report, PAD, evidence sufficiency, or work authorization
  expectedRoute: BDPF.3B -> BDPF.3 -> BDPF.3A -> BDPF.4/BDPF.5/BDPF.6 -> BDPF.9
...
2. Return to `BDPF.3B` / `BDPF.3` / `BDPF.3A` when source package, carrier inventory, source signals, needKind, source-return conditions, readiness gate, or affected structures change.
...
| `D5PackageFormLayeringAndRelationAdequacy` | 3 | Stewardship, dependency, PFAD, seven PFR records | E.21 pattern evidence still lower/not materialized. |
...
| `D8HeterogeneousCaseAndTransferAdequacy` | 3 | Eight acceptance cases | Replace synthetic cases with pilot evidence. |
...
CC-BDPF-PKG-2: DPFPackageStewardshipRecord, FrameworkEditionDependencyRecord, PFAD, and at least seven PFR records are filled.
CC-BDPF-PKG-3: Pattern ids are package-local BDPF.*, with legacy alias table.
CC-BDPF-PKG-4: SourceUseAndRefreshMap exists and at least one source row affects each of BDPF.3, 3A, 3B, 4, 5, 6, 7, 8, 10, 11.
